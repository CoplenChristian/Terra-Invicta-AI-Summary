// shared/prng.mjs
//
// Purpose: the deterministic Mulberry32 PRNG and percentile sampler shared by
//   every seeded simulation, in both runtimes.
//
// Moved here from `server/commentary/prng.js` on 2026-08-22 so the Cloudflare
// worker can run the same seeded engagement sweep the Node server does. The
// code is unchanged; `server/commentary/prng.js` is now a CommonJS barrel
// re-exporting these same function objects, so no caller moved.
//
...
//
// Plain ESM, no Node built-ins, no imports -- the hosted worker cannot
// `require` CommonJS.
 ...
 */
export function samplePercentile(sortedValues, p) {
