/**
 * scripts/verify_command_grid2.js
 *
 * Purpose: six post-conversion checks for COMMAND TwoColumnGrid migration.
 */
...
async function run() {
  ensureBundleBuilt();
  const beforePath = path.resolve('tests/fixtures/command-view-proof/baseline-before.json');
  const afterPath = path.resolve('tests/fixtures/command-view-proof/baseline-after.json');
  ...
  try {
    for (const mode of MODES) {
      for (const vp of VIEWPORTS) {
        ...
        const result = await page.evaluate((panelIds) => {
          function rectsOverlap(a, b) {
            return a.x < b.x + b.width && a.x + a.width > b.x && a.y < b.y + b.height && a.y + a.height > b.y;
          }
          ...
          const kpi = document.querySelector('#view-command .init-kpi-banner');
        ...
        // Check 6: KPI banner full width at desktop
        if (vp.label === 'desktop' && mode === 'player') {
          const contentWidth = Math.min(1660, result.innerWidth);
          if (result.kpiWidth < contentWidth * 0.95) {
            failures.push(`[6 full-width] KPI banner width ${result.kpiWidth} < 95% of content ${contentWidth}`);
          }
        }
  ...
  console.log('[6] KPI banner full-width at desktop:', failures.filter((f) => f.startsWith('[6')).length === 0 ? 'PASS' : 'FAIL');
  ...
  console.log('\nAll six COMMAND grid2 checks passed.');
}
