/**
 * scripts/verify_drives_grid2.js
 *
 * Purpose: six post-conversion checks for DRIVES TwoColumnGrid migration.
 */
...
const SPAN_PANEL_IDS = ['driveExplorer'];
...
async function run() {
  ensureBundleBuilt();
  const beforePath = path.resolve('tests/fixtures/drives-view-proof/baseline-before.json');
  const afterPath = path.resolve('tests/fixtures/drives-view-proof/baseline-after.json');
  ...
  try {
    for (const mode of MODES) {
      for (const vp of VIEWPORTS) {
        ...
        await page.goto(`http://127.0.0.1:${port}/v2/index.html#/drives`, { waitUntil: 'networkidle' });
        await page.evaluate((targetMode) => {
          document.querySelector(`.init-mode-btn[data-mode="${targetMode}"]`)?.click();
        }, mode);
        await page.waitForTimeout(1200);
        await page.evaluate(() => {
          document.querySelector('.init-nav-btn[data-view="drives"]')?.click();
        });
        ...
        const result = await page.evaluate(({ panelIds, spanPanelIds }) => {
          ...
          function findPanelCard(el) {
            if (!el) return null;
            return el.closest('.tech-card')
              || el.closest('[data-primitive="panel"]')
              || el.querySelector('.tech-card, [data-primitive="panel"]');
          }

          const section = document.getElementById('view-drives');
        ...
        // Check 6: previously full-width panels stay full-width at desktop (by WIDTH)
  ...
  console.log('\nAll six DRIVES grid2 checks passed.');
}
