/**
 * scripts/verify_records_grid2.js
 *
 * Purpose: six post-conversion checks for RECORDS TwoColumnGrid migration.
 */
...
const SPAN_PANEL_IDS = ['unlockedTech', 'sinceLastSave'];
...
async function run() {
  ensureBundleBuilt();
  const beforePath = path.resolve('tests/fixtures/records-view-proof/baseline-before.json');
  const afterPath = path.resolve('tests/fixtures/records-view-proof/baseline-after.json');
  ...
  try {
    for (const mode of MODES) {
      for (const vp of VIEWPORTS) {
        ...
        await page.goto(`http://127.0.0.1:${port}/v2/index.html#/records`, { waitUntil: 'networkidle' });
        await page.evaluate((targetMode) => {
          document.querySelector(`.init-mode-btn[data-mode="${targetMode}"]`)?.click();
        }, mode);
        await page.waitForTimeout(1200);
        await page.evaluate(() => {
          document.querySelector('.init-nav-btn[data-view="records"]')?.click();
        });
        ...
        if (!result.apiAccess || result.apiAccess.linkCount !== 6) {
          failures.push(`[api] ${mode}/${vp.label}: API access block missing or wrong link count`);
        }
        if (result.apiAccess?.ariaLabelledby !== 'initApiAccessTitle' || result.apiAccess?.titleId !== 'initApiAccessTitle') {
          failures.push(`[api a11y] ${mode}/${vp.label}: aria-labelledby / title id broken`);
        }
  ...
  console.log('\nAll six RECORDS grid2 checks passed.');
}
