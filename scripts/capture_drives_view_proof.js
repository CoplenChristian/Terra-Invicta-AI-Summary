/**
 * scripts/capture_drives_view_proof.js
 *
 * Purpose: capture DRIVES view rendered text and panel-mount geometry for
 *   before/after refactor proof. Adapted from capture_fleet_view_proof.js.
 *
 * The drive catalogue is lazy-loaded on first view activation — this script
 * waits for #driveExplorer .de-table before measuring so an unloaded shell is
 * never compared against a loaded panel.
 */
  ...
  try {
    for (const mode of MODES) {
      ...
      for (const vp of VIEWPORTS) {
        ...
        await page.goto(`http://127.0.0.1:${port}${SHELL_PATH}#/drives`, {
        ...
        await page.evaluate(() => {
          const btn = document.querySelector('.init-nav-btn[data-view="drives"]');
          if (btn) btn.click();
        });
        ...
        result.modes[mode].viewports[vp.label] = await page.evaluate((panelIds) => {
          ...
          if (!section) return { error: 'view-drives missing' };
