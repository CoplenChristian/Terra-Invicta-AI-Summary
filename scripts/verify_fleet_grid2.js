/**
 * scripts/verify_fleet_grid2.js
 *
 * Purpose: six post-conversion checks for FLEET TwoColumnGrid migration.
 */
...
const SPAN_PANEL_IDS = ['fleetProcurement'];
...
async function run() {
  ensureBundleBuilt();
  const beforePath = path.resolve('tests/fixtures/fleet-view-proof/baseline-before.json');
  const afterPath = path.resolve('tests/fixtures/fleet-view-proof/baseline-after.json');
  ...
  try {
    for (const mode of MODES) {
      for (const vp of VIEWPORTS) {
        ...
        await page.goto(`http://127.0.0.1:${port}/v2/index.html#/fleet`, { waitUntil: 'networkidle' });
        await page.evaluate((targetMode) => {
          document.querySelector(`.init-mode-btn[data-mode="${targetMode}"]`)?.click();
        }, mode);
        await page.waitForTimeout(1200);
        await page.evaluate(() => {
          document.querySelector('.init-nav-btn[data-view="fleet"]')?.click();
        });
  ...
  console.log('\nAll six FLEET grid2 checks passed.');
}
