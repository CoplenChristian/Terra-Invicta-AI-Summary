/**
 * scripts/capture_expansion_view_proof.js
 *
 * Purpose: capture EXPANSION view rendered text and panel-mount geometry for
 *   before/after refactor proof. Adapted from capture_command_view_proof.js.
 */
  ...
  try {
    for (const mode of MODES) {
      ...
      for (const vp of VIEWPORTS) {
        ...
        await page.goto(`http://127.0.0.1:${port}${SHELL_PATH}#/expansion`, {
        ...
        await page.evaluate(() => {
          const btn = document.querySelector('.init-nav-btn[data-view="expansion"]');
          if (btn) btn.click();
        });
        ...
        result.modes[mode].viewports[vp.label] = await page.evaluate((panelIds) => {
          ...
          if (!section) return { error: 'view-expansion missing' };
