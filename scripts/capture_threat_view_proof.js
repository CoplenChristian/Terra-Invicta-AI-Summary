/**
 * scripts/capture_threat_view_proof.js
 *
 * Purpose: capture THREAT view rendered text and panel-mount geometry for
 *   before/after refactor proof. Writes JSON to stdout or --out file.
 */
...
function captureThreatView() {
  const section = document.getElementById('view-threat');
  if (!section) return { error: 'view-threat missing' };
  ...
  try {
    for (const mode of MODES) {
      ...
      for (const vp of VIEWPORTS) {
        ...
        await page.goto(`http://127.0.0.1:${port}${SHELL_PATH}#/${mode === 'player' ? '' : ''}threat`, {
        ...
        await page.evaluate(() => {
          const btn = document.querySelector('.init-nav-btn[data-view="threat"]');
          if (btn) btn.click();
        });
...
module.exports = { runCapture, captureThreatView, PANEL_IDS, VIEWPORTS, MODES };
