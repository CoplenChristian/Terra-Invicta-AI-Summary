...
// Re-exported below so every existing caller and test keeps importing the
// capability comparison from here. `isAlienFaction` is the one name this file
// still calls directly: classifyProxy's alien branch delegates to it, so "who
// are the aliens" has a single definition shared with the fleet comparison.
const {
  CAPABILITY_AXES,
...
} = require('../shared/fleetCapability.mjs');
...
// The capability axes, the decisive ratio, the median helpers and
// summarizeFleetCapability now live in shared/fleetCapability.mjs so that
// /api/intel/research-ranking orders its military candidates against THIS
// deficit computation rather than a second one of its own. They are
// re-exported unchanged at the bottom of this file, so every existing
