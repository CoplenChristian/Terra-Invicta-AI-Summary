...
| `server/intelligenceFilter.js` | C | Node (CommonJS) | 929 | redact and filter a raw snapshot down to what the observer is allowed to see. | — | `tests/intelligenceFilter.test.js` |
...
| `server/snapshot/rawSnapshot.js` | C | Node (CommonJS) | 237 | the orchestration pass — read the save's collections once, build the | `ASSUMED_CAMPAIGN_START_YEAR, buildRawSnapshot` | — |
