...
| `public/v2/js/components/mining-expansion.js` | BS | Browser (ESM) | 419 | the mining expansion board — capacity, runways, and need-weighted | — | — |
| `public/v2/js/components/research-advisor.js` | BS | Browser (ESM) | 1044 | phase 4 of the research advisor, on screen. | — | — |
...
| `server/briefing/readers.js` | C | Node (CommonJS) | 238 | snapshot-shaped readers — the joins and roll-ups the briefing and | `buildAdvisableHabs, getControlledNationData, getFleetCombatPower, getMiningRateSummary, getResearchSlots, isFilteredDataAvailable, readObserverHateTrend` | — |
...
| `server/commentary/simulation.js` | C | Node (CommonJS) | 315 | Layer 3 of the strategic commentary — the opponent-tier builders and | `BATTLE_TRIALS_PER_COUNT, MAX_SIMULATED_HULLS, OPPONENT_RATING_BASIS, SIMULATION_SEEDS_COUNT, TARGET_WIN_PROBABILITY, buildOmniscientOpponentTiers, buildPlayerOpponentTiers, describeBandUncertainty, findRequiredHullsForTier, guaranteedWinHullCount, runMonteCarloSimulation` | — |
...
| `server/http/routes/snapshot.js` | C | Node (CommonJS) | 189 | routes returning the whole filtered snapshot or a rendering of it, | `register, registerReadOnlyExports` | — |
...
| `server/intelligenceFilter.js` | C | Node (CommonJS) | 946 | redact and filter a raw snapshot down to what the observer is allowed to see. | — | `tests/intelligenceFilter.test.js` |
...
| `server/snapshot/factions.js` | C | Node (CommonJS) | 478 | the faction-level roll-up — relationships, resources and income, | `buildActiveXenoforming, buildAlienFacilities, buildDefaultTargets, buildFactionIntelligence, buildFactionRelationships, buildFactions, buildTechMatrix, collectShipDesigns, normalizeFactionIntelligence` | — |
...
| `server/snapshot/rawSnapshot.js` | C | Node (CommonJS) | 267 | the orchestration pass — read the save's collections once, build the | `ASSUMED_CAMPAIGN_START_YEAR, buildRawSnapshot` | — |
...
| `shared/campaignElapsed.mjs` | E | Node + Cloudflare worker (ESM) | 176 | resolve elapsed campaign time and alien progression speed from one | `CAMPAIGN_AGE_SOURCES, DAYS_PER_CAMPAIGN_YEAR, resolveAlienProgressionSpeed, resolveCampaignElapsed` | `tests/campaignElapsed.test.js` |
| `shared/campaignSettings.mjs` | E | Node + Cloudflare worker (ESM) | 336 | parse the save's custom-difficulty block and label a customised | `CAMPAIGN_SETTINGS_UNAVAILABLE, CAMPAIGN_SETTING_FIELDS, CAMPAIGN_SETTING_NUMERIC_FIELDS, CAMPAIGN_SETTING_VERDICTS, SETTING_KINDS, buildCampaignSettings, describeCampaignDifficulty, formatCampaignSettingValue, parseCampaignSettingFlag, parseCampaignSettingNumber` | `tests/campaignSettings.test.js` |
...
| `shared/engagementModel.mjs` | E | Node + Cloudflare worker (ESM) | 293 | the seeded Monte Carlo engagement sweep — one hull-count model, | `BAND_EXCLUSIONS, BATTLE_TRIALS_PER_COUNT, GUARANTEED_WIN_MARGIN, MAX_SIMULATED_HULLS, OPPONENT_RATING_BASIS, SIMULATION_SEEDS_COUNT, TARGET_WIN_PROBABILITY, describeBandUncertainty, findRequiredHullsForTier, guaranteedWinHullCount, simulateEngagement` | — |
...
| `shared/fleetEngagement.mjs` | E | Node + Cloudflare worker (ESM) | 987 | per-fleet engagement estimates — what force each specific alien | `COMPOSITION_BASIS, DEFAULT_ENGAGEMENT_ROWS, ENGAGEMENT_ORDERED_BY, ENGAGEMENT_VERDICTS, FLEET_REACHABILITY_STATES, MAX_ENGAGEMENT_HULLS, PLAYER_ANCHOR_EVIDENCE, buildFleetEngagement` | `tests/fleetEngagement.test.js` |
...
| `shared/intel/common.mjs` | E | Node + Cloudflare worker (ESM) | 212 | the primitives every intel projection shares — the mining resource | `COMBAT_POWER_SOURCE, MINING_RESOURCES, bodyMatches, combatPowerTotal, destinationMatches, factionMatches, findAlienFaction, normalizeBody, normalizeCostObject, rateMultiplier, siteMonthlyOutput, zeroedBySaveKey` | — |
...
| `shared/intel/driveExplorer.mjs` | E | Node + Cloudflare worker (ESM) | 1090 | /api/intel/drive-explorer — every drive in the catalogue rated | `DEFAULT_DRIVE_SORT, DRIVE_AVAILABILITY, DRIVE_SORTS, MEASUREMENT_BASIS, driveExplorerResource` | `tests/driveExplorer.test.js` |
...
| `shared/intel/militaryValue.mjs` | E | Node + Cloudflare worker (ESM) | 1480 | /api/intel/military-value — phase 2 of the research advisor, pricing | `militaryValueResource` | `tests/militaryValue.test.js` |
...
| `shared/intel/propulsion.mjs` | E | Node + Cloudflare worker (ESM) | 495 | /api/intel/propulsion — phase 1 of the research advisor, pairing the | `propulsionResource` | — |
...
| `shared/miningTechBonus.mjs` | E | Node + Cloudflare worker (ESM) | 395 | the observer's per-resource mine-output multipliers from completed | `ADDITIVE_MINING_BONUS_PROJECTS, MINING_BONUS_MEASURED_ON, MINING_BONUS_RULES, MINING_BONUS_STACKING, MINING_BONUS_STATES, UNMODELLED_FACTORS, applyMiningTechBonus, buildMiningTechBonuses, miningTechBonusCaveat` | `tests/miningTechBonus.test.js` |
...
| `shared/researchAvailability.mjs` | E | Node + Cloudflare worker (ESM) | 389 | which projects a faction can actually research, in the three states | `AVAILABILITY_STATES, buildAvailabilityResolver, getPrerequisiteBranches, monthsAtIncome, tallyAvailabilityStates` | — |
| `shared/researchCategoryBonus.mjs` | E | Node + Cloudflare worker (ESM) | 1029 | the observer's per-category research bonuses from all five wiki-named | `CATEGORY_BONUS_RULES, CATEGORY_BONUS_SOURCE_TYPES, CATEGORY_BONUS_STATES, CATEGORY_DURATION_BASES, CATEGORY_DURATION_STATES, CATEGORY_RATE_MODEL, MEASURED_INCOME_BASIS, UNHANDLED_SOURCE_TYPES, applyDiminishingReturns, buildResearchCategoryBonuses, categoryBonusCaveat, categoryBonusSummary, …(+1)` | `tests/researchCategoryBonus.test.js` |
...
| `shared/researchSlots.mjs` | E | Node + Cloudflare worker (ESM) | 507 | research-advisor phase-5 allocation of research to slots. | `ALLOCATION_MODEL, SLOT_INDEX_PIN, SLOT_KINDS, SLOT_KIND_LABELS, buildResearchSlotAllocation` | `tests/researchSlots.test.js` |
...
| `shared/strategicSnapshot.mjs` | E | Node + Cloudflare worker (ESM) | 613 | reduce a full raw snapshot to a compact strategic_snapshot_v1 | `DEFAULT_HISTORY_POLICY, HAB_CONSTRUCTION_MODULES, MINE_LIMIT_GRANTS, SHIP_CONSTRUCTION_MODULES, STRATEGIC_SNAPSHOT_SCHEMA, STRATEGIC_SNAPSHOT_VERSION, buildStrategicSnapshot, deriveEvents` | `tests/strategicSnapshot.test.js` |
