...
**216 JS modules** and **24 stylesheet parts** (240 indexed files).
...
| `server/intelligenceFilter.js` | C | Node (CommonJS) | 1052 | redact and filter a raw snapshot down to what the observer is allowed to see. | — | `tests/intelligenceFilter.test.js` |
...
| `server/snapshot/rawSnapshot.js` | C | Node (CommonJS) | 362 | the orchestration pass — read the save's collections once, build the | `ASSUMED_CAMPAIGN_START_YEAR, buildRawSnapshot` | — |
| `server/snapshot/research.js` | C | Node (CommonJS) | 149 | global research — the shared tech slots every faction contributes | `buildGlobalResearchSlots, buildTechTree` | — |
| `server/snapshot/space.js` | C | Node (CommonJS) | 735 | everything above the atmosphere — fleets and ships, habs, hab sites | `buildFleets, buildHabModules, buildHabSites, buildHabs, buildResourceTransfers, buildShipyardQueues, buildShipyardStations, buildWeaponLoadout, classifyHabModule, daysRemainingForStatus, formatWeaponSummary, getDominantWeaponType, …(+10)` | — |
| `server/snapshot/templates.js` | C | Node (CommonJS) | 1064 | the template-derived static builders — they read the installed game | `MISSION_HATE_SLOT, UNLOCK_FAMILIES, WEAPON_FAMILIES, buildComponentStats, buildDriveStats, buildEffectIndex, buildMissionSpecs, buildProjectGating, buildPropellantModules, buildShipHullStats, buildTechBonusCatalogue, buildTraitStatMods, …(+1)` | — |
...
| `src/v2/panels/FleetProcurement.jsx` | E | Browser (React JSX) | 408 | the FLEET view — what is already unlocked but not in service, and the | `FleetProcurement, RefitDesignCard` | — |
| `src/v2/panels/fleetProcurementUtils.mjs` | E | Browser (React JSX) | 712 | the DOM-free half of the FLEET procurement + refit advisor panel — | `ARMOR_DATA, ARMOR_RATIO_UNMEASURED_TEXT, DELIVERY_FAILS_TITLE, DELIVERY_UNKNOWN_TITLE, DESIGN_ROLES, NON_COMPOSABILITY_NOTICE, NOTHING_UNFIELDED, NO_ENDPOINT_ANSWER, PROCUREMENT_UNAVAILABLE_HEADLINE, REFIT_NOTICE, RULE_SCALAR_KIND, RULE_SCALAR_TITLE, …(+28)` | — |
...
| `src/v2/primitivesHarness.jsx` | E | Browser (React JSX) | 651 | browser-test mount point for Track E primitives — not loaded by the | `HarnessApp, SCENES` | — |
