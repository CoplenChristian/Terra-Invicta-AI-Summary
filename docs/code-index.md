...
| `public/v2/js/mission-control.js` | BS | Browser (classic, global IIFE) | 2174 | the v2 dashboard controller — briefing fetch, mode switching, and | `onCopyExport, onOpenFaction` | — |
...
| `scripts/run_unit_tests.js` | C | Node (CommonJS) | 89 | run the deterministic unit test suite — every tests/**/*.test.js file | — | — |
...
| `scripts/verify_drives_grid2.js` | C | Node (CommonJS) | 218 | six post-conversion checks for DRIVES TwoColumnGrid migration. | — | — |
...
| `server/engine/military.js` | C | Node (CommonJS) | 730 | the military read-model — assemble the observer's military facts into | `MODULE_TIER_SOURCE_TABLE, MODULE_TIER_SOURCE_UNKNOWN, buildMilitaryWorld, buildTheaterForce, resolveModuleTier` | — |
...
| `server/http/routes/snapshot.js` | C | Node (CommonJS) | 217 | routes returning the whole filtered snapshot or a rendering of it, | `register, registerReadOnlyExports` | — |
...
| `server/intelligenceFilter.js` | C | Node (CommonJS) | 1114 | redact and filter a raw snapshot down to what the observer is allowed to see. | — | `tests/intelligenceFilter.test.js` |
...
| `server/saveParser.js` | C | Node (CommonJS) | 276 | locate, decompress and parse a Terra Invicta save file into raw game state. | — | — |
...
| `server/snapshot/rawSnapshot.js` | C | Node (CommonJS) | 412 | the orchestration pass — read the save's collections once, build the | `ASSUMED_CAMPAIGN_START_YEAR, buildRawSnapshot` | — |
...
| `server/snapshot/space.js` | C | Node (CommonJS) | 800 | everything above the atmosphere — fleets and ships, habs, hab sites | `buildFleets, buildHabModules, buildHabSites, buildHabs, buildResourceTransfers, buildShipyardQueues, buildShipyardStations, buildWeaponLoadout, classifyHabModule, daysRemainingForStatus, formatWeaponSummary, getDominantWeaponType, …(+12)` | — |
...
| `shared/battleComposition.mjs` | E | Node + Cloudflare worker (ESM) | 733 | phase 0 + phase 1 of docs/engagement-matchup-spec.md — the weapon | `INTERCEPTION_ASSUMPTION, MAX_BATTLE_SIDE_SHIPS, MAX_BATTLE_SIDE_SHIPS_ATTRIBUTION, PD_OVERWHELM_MULTIPLE, PD_OVERWHELM_RULE_ATTRIBUTION, PD_TARGETABLE_FAMILY_FALLBACK, SALVO_SHOTS_WHEN_ABSENT, WEAPON_FAMILIES, buildWeaponIndex, composeBattleSide, normalizeWeaponName, saturationVerdict, …(+1)` | `tests/battleComposition.test.js` |
...
| `shared/campaignSettings.mjs` | E | Node + Cloudflare worker (ESM) | 784 | parse the save's two campaign-customisation blocks and label a | `CAMPAIGN_SETTINGS_UNAVAILABLE, CAMPAIGN_SETTING_FIELDS, CAMPAIGN_SETTING_NUMERIC_FIELDS, CAMPAIGN_SETTING_VERDICTS, SCENARIO_CUSTOMIZATIONS_UNAVAILABLE, SCENARIO_CUSTOMIZATION_BOOLEAN_FIELDS, SCENARIO_CUSTOMIZATION_FIELDS, SCENARIO_CUSTOMIZATION_NUMERIC_FIELDS, SETTING_KINDS, buildCampaignSettings, buildScenarioCustomizations, deriveArmourMultipliers, …(+7)` | `tests/campaignSettings.test.js` |
...
| `src/v2/panels/FleetProcurement.jsx` | E | Browser (React JSX) | 704 | the FLEET view — what is already unlocked but not in service, and the | `FleetProcurement, RefitDesignCard` | — |
| `src/v2/panels/fleetProcurementUtils.mjs` | E | Browser (React JSX) | 783 | the DOM-free half of the FLEET procurement + refit advisor panel — | `ARMOR_DATA, ARMOR_RATIO_UNMEASURED_TEXT, DELIVERY_FAILS_TITLE, DELIVERY_UNKNOWN_TITLE, DESIGN_ROLES, NON_COMPOSABILITY_NOTICE, NOTHING_UNFIELDED, NO_ENDPOINT_ANSWER, NO_REFIT_CANDIDATES, NO_REFIT_ENDPOINT_ANSWER, PROCUREMENT_UNAVAILABLE_HEADLINE, REFIT_FAILURE_ANSWER, …(+32)` | — |
