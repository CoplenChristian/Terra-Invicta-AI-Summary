...
| 3b | **price durations through the pinned allocation model** | **closed 2026-08-22 as NOT TO BE DONE, and no duration moved.** The contradiction is settled: both measurements were real and the *inference* from one was a units error. **2.11× is the observer's four slots delivering `cachedYearlyRevenue.Research` SUMMED**; a duration is about one slot, which measured 0.4658×, 0.2928×, 1.0602× and 0.2928× of the nominal income — and those four sum to the 2.11×. Three are **below 1**, so the flat figure is already 2.15×–3.42× *optimistic* there, not conservative; dividing by 2.11 would take the 1-pip slots to 7.2× optimistic. **`campaign-settings-spec.md` reached the right verdict by invalid reasoning** — a delivered/predicted ratio near 1.0 cannot detect a missing ×2 when the prediction carries a fitted `ProjectBonus` of −0.209, and its `[0,0,3,3,3,0]` / 2033 record does not match the saves it names (`[0,0,3,1,3,1]`, 2034–35). Both documents corrected in place with the superseded reasoning kept and marked. Replacement evidence: measured gain **2.1115×** against **2.1420×** predicted from terms all read from the save, where a pre-200% income would have needed **4.2840×** — so `cachedYearlyRevenue.Research` is already post-`researchSpeedMultiplier`. Every intel endpoint captured in both modes before and after: **zero numeric change**, narrative fields only, and the research-advisor ranking order is byte-identical. Still open, recorded not built: the *fastest-achievable* duration (all pips on one slot) is a real lower bound, `1.05 × (1 + Category + Project)`, but it is category-dependent (1.1025× to 2.5095×) and needs a counterfactual allocation per candidate |
