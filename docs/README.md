...
| 3c | `drive-columns-and-filters-spec.md` | **not started.** DRIVES offers a `CRUISE ACCEL` sort whose column is never rendered, so choosing it reorders the table by an invisible key. Not cosmetic: `combat / cruise` is exactly `thrustCap`, and only **72 of 541** drives have the two equal — for the other 469 the displayed combat figure overstates sustained transit acceleration by up to **160×** (VASIMR x1 reads 0.01010778 combat against 0.00016846 cruise). Part B adds minimum-threshold filters on ΔV / combat / cruise, which must live on `/api/intel/drive-explorer` and not only in `paint()`, or no agent can use them. The risk is `Number(null) === 0` turning an untestable row into a failed one — untestable must be counted separately. Adds an eighth column to a view whose mobile fit was only just repaired in `dda9b25` |
